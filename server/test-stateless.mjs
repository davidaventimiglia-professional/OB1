 ...
 * test-stateless.mjs
 *
 * Validates the per-request McpServer transport pattern without any infra
 * (no Supabase, no DB). MCP initialize is a pure protocol handshake — no tools
 * are called, no database touched.
 *
 * Scope: this harness covers ONLY the stateless transport contract that
 * `server/index.ts` relies on — a fresh `McpServer` + `StreamableHTTPTransport`
 * per request, CORS headers on every response, and the stripped
 * `mcp-session-id` header. It deliberately does NOT exercise authentication.
 *
 * The core server is now an OAuth 2.1 resource server: it validates Bearer JWTs
 * issued by Supabase Auth (see `server/auth.ts` + `server/index.ts`). The static
 * `x-brain-key` / `MCP_ACCESS_KEY` model has been retired. Real auth behavior
 * (JWKS resolution, signature/issuer/expiry verification, `client_id` binding,
 * the 401 + `WWW-Authenticate` challenge, and RFC 9728 discovery) is covered by
 * `server/auth_test.ts` and the manual verification checklist in `docs/auth.md`.
 * A full OAuth handshake needs a live authorization server, so it is not
 * simulated here.
 *
...
// ── Minimal server mirroring the per-request transport pattern ───────────────
...
// No auth gate here on purpose — auth is an OAuth concern tested elsewhere.
// This handler isolates the stateless per-request transport behavior.
app.all("*", async (c) => {
...
console.log("\n[2] MCP initialize — response shape + no mcp-session-id");
{
  const r = await fetch(BASE, { method: "POST", headers: mcpHeaders, body: INIT });
...
console.log("\n[3] Per-request isolation — two sequential initializes");
{
  const r1 = await fetch(BASE, { method: "POST", headers: mcpHeaders, body: INIT });
  ...
  const r2 = await fetch(BASE, { method: "POST", headers: mcpHeaders, body: INIT });
...
console.log("\n[4] tools/list — verifies buildServer() registers tools each time");
{
  const listMsg = JSON.stringify({ jsonrpc: "2.0", id: 2, method: "tools/list", params: {} });
  const r = await fetch(BASE, { method: "POST", headers: mcpHeaders, body: listMsg });
